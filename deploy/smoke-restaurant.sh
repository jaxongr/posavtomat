...
echo "== Order lifecycle: open occupies table, pay frees it =="
T2=$(curl -sS "${H[@]}" "$API/tables" | jq -r '.data[]|select(.status=="FREE")|.id' | head -1)
OPEN=$(curl -sS -X POST "${H[@]}" -H 'Content-Type: application/json' "$API/orders" -d "{\"tableId\":\"$T2\",\"items\":[{\"productId\":\"$DISH\",\"qty\":1}]}")
O2=$(echo "$OPEN" | jq -r '.data.id'); O2TOTAL=$(echo "$OPEN" | jq -r '.data.total')
...
curl -sS -X POST "${H[@]}" -H 'Content-Type: application/json' "$API/orders/$O2/pay" -d "{\"payments\":[{\"provider\":\"CASH\",\"amount\":$O2TOTAL}]}" >/dev/null 2>&1
TS2=$(curl -sS "${H[@]}" "$API/tables" | jq -r ".data[]|select(.id==\"$T2\")|.status")
[ "$TS2" = "FREE" ] && ok "hisob to'langach stol bo'shadi" || bad "pay free: $TS2"
